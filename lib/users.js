module.exports = function (redis, config) {
  ...
  function storeFollowers(ids, screen_name) {
    ...
    function renameToOld() {
      if (flagNoDiff || (flagDiff1 && flagDiff2)) //if all the operations using the set finished
  ...
  User.prototype.followersDataRecent = function () {
    var cache_period = config.cache_period || 5
    return moment().subtract('minutes', cache_period) < moment(this.lastFollowersDataRetrieved)
  };
