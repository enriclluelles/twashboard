module.exports = function (redis, config) {
  ...
  , util = require('util')
  , follower_retrieval = require('./follower_retrieval')(redis, config, User)
  ...
  User.prototype.getFollowersData = function getFollowersData(as_user, cb) {
  ...
  User.prototype.getFollowers = function getFollowers(cb) {
    ...
    if ((process.env.NODE_ENV == 'production') && self.isFollowersRecent()) {
      follower_retrieval.followerListFromRedis(self.uid, aux);
    } else {
