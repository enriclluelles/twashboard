module.exports = function(redis, config, User) {
  ...
  function followerListFromTwitter(as_user, from_user, cb) {
    twitter.getFollowers(from_user.uid, as_user.getTokenPair(), function (followers) {
      storeFollowers(followers, as_user.uid);
  ...
  function followerData(as_user, from_user, cb) {
    ...
    followerListFromRedis(from_user.uid, function(fwIds){

      keys = fwIds.map(function (id){ return "users:" + id});
      ...
      redis.mget(keys, function (err, users_from_redis) {
        ...
        twitter.lookupUsers(unknown_users, as_user.getTokenPair(), function (followers) {
  ...
  function getFollowerHistory(user, as_user, cb) {
    ...
    redis.zrevrange('diff:' + user.uid, 0, -1, 'WITHSCORES', function (err, values) {
